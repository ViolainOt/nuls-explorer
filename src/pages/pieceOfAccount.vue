        ...
        <div class="mobile-auto-fix">
            ...
            <table boeder="1" class="nuls-ul-table">
                ...
                <tr>
                    ...
                    <th>{{$t('outPieceAccount.number')}}</th>
                    <th>{{$t('outPieceAccount.address')}}</th>
                    <th>{{$t('outPieceAccount.blockReward')}}</th>
                    <th>{{$t('outPieceAccount.reward')}}</th>
                    <th>{{$t('outPieceAccount.lastRewardBlock')}}</th>
                    <th class="space-th"></th>
                </tr>
                </thead>
                ...
                <tr v-for="(block,key) in blockList">
                    ...
                    <td :data-label="$t('outPieceAccount.number')">{{((currentPage-1)*pageSize)+key+1}}</td>
                    <td :data-label="$t('outPieceAccount.address')"><router-link :to="{path:'/consensusNode',query:{address:block.packingAddress,type:1}}">{{block.packingAddress}}</router-link></td>
                    <td :data-label="$t('outPieceAccount.blockReward')">{{block.totalPackingCount}}</td>
                    <td :data-label="$t('outPieceAccount.reward')">{{block.totalReward | getInfactCoin}}</td>
                    <td :data-label="$t('outPieceAccount.lastRewardBlock')" class="td-last">{{block.lastRewardHeight}}</td>
                    <td class="space-td"></td>
                </tr>
                </tbody>
                <tbody v-else>
                ...
                <tr class="small-show">
                    <td :data-label="$t('outPieceAccount.number')">{{ $t('notice.noMessage') }}</td>
                    <td :data-label="$t('outPieceAccount.address')">{{ $t('notice.noMessage') }}</td>
                    <td :data-label="$t('outPieceAccount.blockReward')">{{ $t('notice.noMessage') }}</td>
                    <td :data-label="$t('outPieceAccount.reward')">{{ $t('notice.noMessage') }}</td>
                    <td :data-label="$t('outPieceAccount.lastRewardBlock')">{{ $t('notice.noMessage') }}</td>
                </tr>
                </tbody>
            </table>
